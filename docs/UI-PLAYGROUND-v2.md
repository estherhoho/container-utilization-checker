...
**Primary color:** Calm professional blue — #1677FF (Ant Design default). Used for: active toggles, submit button fill, active progress bar, focus rings. No gradient, no tints.

**Result card density:** Sparse. Each result metric gets its own full-width row with 20px vertical spacing. Verdict badge on its own row (large). Recommendation on its own row. Volume bar on its own row. Payload bar on its own row. Action text block on its own row. Generous white space between each block.

**Typography emphasis:** Display verdict. The "FIT / DOES NOT FIT / TIGHT" verdict badge is 32px bold and dominates the result card. All other result text is 14–16px regular/semibold. The hierarchy is clear: you see the verdict first, then you read the details.

**Iconography level:** Light. Only functional status icons in the result: ✓ (green, fit), ✗ (red, no-fit), ⚠ (orange, tight/warning). No icons in the form. No icons in the onboarding guide card. Clean, utilitarian.

**Intended user feel:** "Trustworthy planning tool." A logistics professional or FBA seller who uses spreadsheets daily will find this familiar. No visual noise. The design says: "I respect your time. Here is your answer."

**Onboarding card style (Option A variant):** The top guide card uses a thin left-border accent (#1677FF, 3px) on a white card with a subtle bottom separator. Minimal, consistent with the single-column calculator aesthetic.

**Real-time hints style (Option A variant):** Hint lines appear as plain text in 12px #8C8C8C below their respective field groups — matching the existing helper text style. Sanity tags appear as small text pills with a light border (no filled background), e.g., `[ ✓ Standard FBA carton ]` in green border + green text.
...
**Primary color:** Teal/green — #0D9E8A. Used for: active toggles, submit button fill, active metric card borders, chart bar fill for "used capacity." Provides a distinct visual signature from Option A's blue. Form labels use this color for section-header small caps.

**Result card density:** Dense. The 2×2 metric card grid shows 4 data points simultaneously in a compact grid. Cards have 12px internal padding (vs 20px in Option A). The CBM Summary card stacks 3 statistics vertically in one card. This layout shows maximum information in the right panel viewport.

**Typography emphasis:** Equal-weight metrics. The fit verdict badge and CBM/payload statistics are all rendered at the same visual weight (24px bold). There is no single dominant number — the dashboard communicates: "Here is all your data, side by side." Users scan horizontally across all 4 cards.

**Iconography level:** Medium. Each metric card has an icon in the card header row (container icon for Fit card, volume icon for Volume card, weight icon for Payload, box icon for CBM Summary). Alert icons (info, warning, error) precede each recommendation text. The chart uses colored fill as its own "icon" — no additional icon in the chart area.

**Intended user feel:** "Professional operations dashboard." An ops manager who works in logistics software or a freight analytics portal daily will feel at home. The teal color differentiates from generic "blue SaaS" and signals a specialized tool. Dense, data-rich, credible.

**Onboarding card style (Option B variant):** The guide card uses the Dashboard's left-panel full-width placement, with a teal (#0D9E8A) icon and a structured 3-step numbered list in a slightly shaded (#F5FEFC) background. Visually integrates with the dashboard's section-header small-caps aesthetic.

**Real-time hints style (Option B variant):** Hints appear in the right panel result area, updating live in the 4-metric card grid. As user types, the "CBM Summary" card animates its values (counter-style update). Sanity tags appear as filled pills in the top-right corner of the relevant metric card (e.g., "Standard FBA carton" pill floats in the Volume Utilization card).
...
**Primary color:** Warm amber — #F5A623. Used for: active step indicator, "Calculate →" CTA button fill, selected container radio card highlight color, and the stepper progress bar fill for completed steps. Amber evokes warmth, approachability, and guided action — distinct from blue tools and green dashboards.

**Result card density:** Medium. Step 3 (Result) is a full-viewport result screen with a single column. Metrics are spaced comfortably — neither as sparse as Option A nor as grid-dense as Option B. The result occupies 100% viewport height on mobile, giving each piece of information room to breathe without excess padding.

**Typography emphasis:** Guided headline. The step heading at 24px bold changes color dynamically ("Your container fits." in green / "Your container is tight." in orange / "Your carton does not fit." in red). This headline IS the verdict. Below it, metrics at 18–20px give supporting context. The hierarchy follows the guided flow: headline → proof → details.

**Iconography level:** Heavy. Includes: step indicator icons (check circle for completed steps, amber circle for active step, gray circle for future steps), a carton dimension SVG diagram in Step 1 showing L/W/H labeled, container type icons (container silhouettes) in the radio cards in Step 2, and directional icons on the navigation buttons (← Back / → Calculate).

**Intended user feel:** "Friendly guided checklist." An SMB ecommerce seller who is new to container planning and fills this out from their phone at a warehouse will find this approachable. The amber warmth signals "I'll walk you through this" rather than "here's your data." Each step feels like a short question, not a complex form.

**Onboarding card style (Option C variant):** The guide card is integrated into Step 1 as a "Step 0" introduction screen that appears BEFORE the first input screen (before carton dimensions). It is a full-card overlay with the 3-step explainer centered, an amber "Get started →" button at the bottom, and a "Skip and go to form" text link. This fits the wizard model — the guide is step zero of the wizard, not a dismissible banner.

**Real-time hints style (Option C variant):** Hints appear inline within each step. In Step 1: carton CBM hint appears below the dimension fields, and sanity tags appear as pill badges in the "Try an example" bar area. In Step 2: density tag appears inline below the container selection (alongside the container spec echo). All hints are styled with amber-adjacent colors — the size tag pill uses amber border + dark text for "Standard FBA carton" to stay consistent with the amber visual identity.
...
With the v0.3 additions (visual differentiation + onboarding + real-time feedback), our recommendation remains **Option A — The Calculator**, but the case for Option B has strengthened.

Option A's sparse single-column layout is the most forgiving context for real-time hints — each hint line appears in a predictable location, and the generous vertical spacing means hints don't crowd the form. The guide card integrates cleanly as a thin left-border card above the form. The blue primary color (#1677FF) is familiar to SMB users from SaaS tools they already use.

Option B gains more from the v0.3 additions than Option A: the dense 2×2 result grid can animate live values as the user types, creating a genuinely impressive data dashboard feel. The teal color (#0D9E8A) makes it visually distinct from every other tool on the Dock to AI site. If Esther wants this tool to feel premium and shareable (users screenshot their results), Option B is now a stronger choice.

Option C's amber aesthetic is the most distinctive but also the most opinionated. It works best if the primary distribution channel is mobile and the primary audience is first-time users. The guide-as-step-zero pattern is elegant for mobile but adds a friction layer for repeat users.

**Esther: please select your preferred visual direction and we'll finalize the developer spec.**
