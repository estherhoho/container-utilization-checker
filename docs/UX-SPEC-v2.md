...
## 9. Visual Differentiation, Onboarding, and Real-Time Feedback (v0.3 additions)

### § 9.A — Visual Differentiation Matrix

The three Interface Playground options differ across five design axes as follows:

| Axis | Option A (Calculator) | Option B (Dashboard) | Option C (Stepper) |
|---|---|---|---|
| Primary color / accent palette | Calm blue `#1677FF` | Teal/green `#0D9E8A` | Warm amber `#F5A623` |
| Result card density | Sparse — one metric per row, generous spacing | Dense — 2×2 metric card grid, tight spacing | Medium — single column but full-viewport result step |
| Form grouping treatment | Plain labels-as-headers, no section dividers | Small-caps uppercase section dividers ("CARTON DIMENSIONS") | Full step screens — each step is a conceptual grouping |
| Typography weight emphasis | Display verdict — "FIT" at 32px bold dominates, metrics secondary | Equal-weight metrics — fit badge and CBM stats both at 24px bold | Guided headline — step heading at 24px bold, result headline color-coded |
| Iconography level | Light — status icons only (✓ × !) in result badge | Medium — icons in metric cards, chart tooltips, alert icons | Heavy — progress step icons, carton dimension diagram, container type icons in radio cards |

At minimum, 3 of the 5 axes vary significantly across all 3 options. All 5 axes vary in this spec. The visual differentiation is additive to the v1 layout strategies (Single Column / Split Two-Column / Stepper Wizard) — layouts remain unchanged from v1.

---

### § 9.B — Onboarding & Instructions

#### § Top Guide Card (First Visit)

A collapsible guide card appears at the very top of the tool on first visit, above the form. It uses `localStorage` to persist dismissed state across sessions.

**Visible state (first visit):**
- Background: `#F0F9FF` (light blue-tinted, distinct from the form card)
- Close/dismiss button: × icon, top-right, 36×36px touch target
- Icon: lightbulb or info icon, left-aligned
- Heading: "How to use this tool" (14px semibold)
- 3-step explainer as numbered list:
  1. "Fill in your carton dimensions, quantity, and weight."
  2. "Pick your container type."
  3. "See if your shipment fits — and what to do if it doesn't."
- Value proposition line below: "Plan your container in under 90 seconds before you book."
- "Try an example" button (see below) — placed inside or directly below the guide card

**Dismissed state:**
- Card is hidden entirely (display: none)
- localStorage key: `cbm_guide_dismissed = true`
- If user clears localStorage or visits in incognito, card appears again

**Re-show trigger:** None in v0.2. Card is one-time dismiss.

---

#### § Per-Field Helper Text

The following helper text strings apply to each field:

| Field | Helper Text |
|---|---|
| Carton Length / Width / Height | "Outer dimensions of the carton in cm or inch." |
| Carton Quantity | "Total number of cartons in this shipment." |
...
Helper text appearance: 12px regular, color `#8C8C8C`, appears below the input field. The weight field helper text renders the "Gross weight, including packaging." segment in semibold (600) weight.

For dimension fields: the helper text "Outer dimensions of the carton in cm or inch." appears once below the entire dimension group (not repeated per field).

---

#### § "Try an Example" Button

**Placement:** Above the form, inside or directly below the guide card. Always visible regardless of guide card state — even after guide is dismissed, the button remains. Position: just above Section 2 (the form input groups), in its own row with 12px top/bottom padding.

**Button label:** `Try an example`
**Style:** Secondary button (outlined), medium size, icon on left: arrow-right-circle or sparkle icon

**Behavior on click:**
- Fills the following fields instantly (no animation delay):
  - Carton Length: 60, unit: cm
  - Carton Width: 40, unit: cm
  - Carton Height: 30, unit: cm
  - Carton Quantity: 500
  - Gross Weight per Carton: 12, unit: kg
  - Container Type: 40GP (pre-selects the 40GP option)
  - Stackable: Yes
- Clears any validation errors currently shown
- User can then edit any field and resubmit
- This simulates a typical SMB FBA scenario: 500 cartons of 60×40×30 cm at 12 kg each in a 40GP

**Why example values matter:**
The example triggers the following real-time feedback (visible once user submits or as they see the prefilled values):
- Total CBM: 36.00 m³ (500 × 0.072 CBM)
- "Standard FBA carton" size tag (60 cm ≤ longest side ≤ 70 cm AND 0.04 ≤ CBM ≤ 0.10 → TRUE for a single carton)
- Actually: 60×40×30 = 0.072 m³ which is in range 0.04–0.10 AND longest side 60 cm is in range 40–70 cm → Standard FBA carton ✓

---

### § 9.C — Real-Time Inline Feedback

#### Trigger Table

| Trigger | Display | Style |
|---|---|---|
| Length + Width + Height all valid | "Carton CBM: X.XXX m³" | Subtle helper line under dimension group |
...
| All 4 carton dims (L/W/H + weight) valid | Carton-size sanity tag | Pill near dimension group |
| Same trigger | Density sanity tag | Pill near weight field |
| Carton dim > container internal dim | "⚠️ Carton too large for selected container" | Inline warning, blocks submit |
...
- **Medium density:** 100–500 kg/m³
- **Dense / heavy cargo:** > 500 kg/m³

#### Tag Visual Treatment

- "✓ Standard FBA carton" — green pill, 12px, font-weight 500, padding 4px 8px, border-radius 10px
- "Large carton" — orange pill, same styling
- "Compact carton" — blue pill, same styling
- "Light cargo (low density)" — blue pill, same styling
- "Medium density" — green pill, same styling
- "Dense / heavy cargo — payload may bind" — red pill, same styling

#### Max 5 Active Hints Rule

At most 5 hint items are shown simultaneously. Priority: warning/error hints appear first. If more than 5 would show, lower-priority advisory hints are hidden until higher-priority ones clear.

#### Calculation Triggers

All calculations fire on `input` event (debounced 150ms). No server call needed — all client-side. If a field loses validity (user clears a value), the dependent hints disappear immediately.

#### Hint Placement

- Carton CBM and size tag hints: appear below the dimension group (Group A), above Group B
- Total CBM and Total weight hints: appear below Group B (Quantity), above Group C (Weight)
- Density tag: appears below Group C (Weight), above Group D (Container Type)
- Carton-too-large warning: appears below the container type selector (Group D), inline with container spec echo
...
**AC-21 — Top guide card dismisses and persists across reloads**
When the user clicks the dismiss (×) button on the top guide card, the card disappears immediately. On page reload, the card does not reappear. The persistence mechanism is localStorage key `cbm_guide_dismissed = true`. Verified by: dismiss the card, inspect localStorage, hard-reload the page, confirm card is not visible.

**AC-22 — "Try an example" button fills all required fields in one click**
Clicking "Try an example" fills the following fields: Length=60cm, Width=40cm, Height=30cm, Quantity=500, Weight=12kg, Container Type=40GP, Stackable=Yes. All required fields are filled; no validation errors appear. User does not need to type anything to submit after clicking the button.

**AC-23 — Live total CBM updates within 100ms of last keystroke**
When valid Length, Width, Height, and Quantity values are all present, the "Total shipment CBM: XX.XX m³" helper line updates with each keystroke in any of those four fields. The update appears within 100ms (debounce ≤ 100ms). Verified by changing the Quantity field value and observing the helper line update.

**AC-24 — Carton-size and density tags appear only when all 4 dimensions valid**
The carton-size sanity tag (Standard FBA / Large / Compact) and density sanity tag (Light / Medium / Dense) appear ONLY when all four values are valid: Length, Width, Height, and Gross Weight per Carton. If any of those four fields is empty or invalid, both tags are hidden. Verified by clearing the Weight field — both tags disappear.

---

*End of UX Specification — Container Utilization Checker / CBM Calculator v0.3*
